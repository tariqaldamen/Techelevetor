...
public class RTNValidator {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		RTNValidator validator = new RTNValidator();

		validator.printApplicationBanner();
		
		File inputFile = validator.getInputFileFromUser();
		try(Scanner fileScanner = new Scanner(inputFile)) {
			while(fileScanner.hasNextLine()) {
				String line = fileScanner.nextLine();
				String rtn = line.substring(0, 9);
				
				if(validator.checksumIsValid(rtn) == false) {
					System.out.println(line + ":   " + rtn + " is an invalid Routing Transit Number!");
				}
				else {
	...
	private boolean checksumIsValid(String routingNumber) {
		System.out.println("Check routing number for: " + routingNumber);
		int checksum = 0;
		for(int i = 0; i < 9; i++) {
