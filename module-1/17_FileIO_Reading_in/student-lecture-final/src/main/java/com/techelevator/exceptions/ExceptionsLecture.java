...
public class ExceptionsLecture {
	...
	public static void main(String[] args) {
	 ...
	 try {
		...
		/*
		 * Create our own exception ...  not really part of the lecture, but cool so I will add if time.
		 */
		try {
			stevesDangerousCode(true);
		} catch (StevesException e) {
	...
	private static void doSomethingDangerous() throws ArrayIndexOutOfBoundsException {
	...
	private static void stevesDangerousCode(boolean throwMyError) throws StevesException {
		 if (throwMyError) {
			 throw new StevesException("Well you went and done it now, dummy!", 100);
		 }
		 else {
			 //do nothing
		 }
